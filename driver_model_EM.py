...
def get_sd_other_lane(sd_next_pred, xy_prev, lane_pred, lanes_rxy, lanes_s):

    lane_c_rxy, lane_d_rxy = lanes_rxy['lane_c_rxy'], lanes_rxy['lane_d_rxy']
    lane_c_s, lane_d_s, sp_c, sp_d = lanes_s['lane_c_s'], lanes_s['lane_d_s'], lanes_s['sp_c'], lanes_s['sp_d']
...
def hallucinate_a_step_change(track_i, lanes_rxy, lanes_s):
    # print('\n###hallucinate_a_step_change###')
    ...
    if track_i[10] == 'c': 
        ...
        offset = track_i[4] + min_dist_bet_lanes 
    print('   *hallucinate_a_step_change => curr_lane: %s, lane_pred:%s, curr_d: %s, curr_vd: %s, offset: %s' % (track_i[10], lane_pred, track_i[4], track_i[6], offset))

    # sd_next_pred, vsvd_next_pred, _ = hallucinate_a_step_no_change(track_i, z)    
    v_lon_pred = track_i[5] #vsvd_next_pred[0]
    ...
    if lane_pred != track_i[10]:  #if next (predicted) lane is different from the current lane
        ...
        _, sd_next_pred = get_sd_other_lane([s_lon_pred, s_lat_pred], veh_xyi, lane_pred, lanes_rxy, lanes_s)
...
def get_accel_neighbors_EM(veh_id, track_ego, z_i, actual_tracks_dict_all_with_headway, mle_probs_final_all_veh, params_range):
    ...
    return [a_ego, a_ego_tilde, a_old, a_new, a_old_tilde, a_new_tilde], veh_id_old, veh_id_new
...
def get_lane_change_prob_EM(veh_id, track_i, z_i, pol_i, lam_i, actual_tracks_dict_all_with_headway, mle_probs_final_all_veh, params_range):
    ...
    [a_ego, a_ego_tilde, a_old, a_new, a_old_tilde, a_new_tilde], veh_id_old, veh_id_new = get_accel_neighbors_EM(veh_id, track_i, z_i, \
        actual_tracks_dict_all_with_headway, mle_probs_final_all_veh, params_range)
    print('   *get_lane_change_prob_EM: => a_ego_tilde', a_ego_tilde, 'a_new_tilde', a_new_tilde)
    # print('       -----------------------------------')
    ...
    if -a_new_tilde <= b_safe:
        ...
        return sigmoid_mobil, mobil, veh_id_old, veh_id_new
    else:
        return 0, 0, veh_id_old, veh_id_new
    
    
def EM(veh_id, track, actual_tracks_dict_all_with_headway, lanes_rxy, lanes_s, params_range, mle_probs_final_all_veh, max_iter_per_veh):
    ...
    for it in range(1, max_iter_per_veh):
        ...
        for i in range(len(track)-1):
            ...
            for z_idx, eps_idx, pol_idx, lam_idx in itertools.product(*idx_range):
                ...
                prob_lane, _, _, veh_id_new = get_lane_change_prob_EM(veh_id, track[i], z_range[z_idx], pol_range[pol_idx], lam_range[lam_idx], \
                ...
                ## if change lane
                # idx_new = np.where(actual_tracks_dict_all_with_headway[veh_id_new][:,0] == track[i][0])
                # track_new_i = actual_tracks_dict_all_with_headway[veh_id_new][idx_new[0], :]
                sd_next_pred_change, _, _ = hallucinate_a_step_change(track[i], lanes_rxy, lanes_s)  #pd controller (lane change) 
...
def run_EM_plot(actual_tracks_dict_all_with_headway, lanes_rxy, lanes_s, max_iter_per_veh, params_range, veh_id_minmax):
    ...
    for veh_id, track in actual_tracks_dict_all_with_headway.items():
        if veh_id >= veh_id_minmax[0] and veh_id <= veh_id_minmax[1]:  #veh_id>= 203 and veh_id < 210:
            ...
            mle_probs_final, mle_probs_history, marginal_ll_final, marginal_ll_history, converged_iter = \
                EM(veh_id, track, actual_tracks_dict_all_with_headway, lanes_rxy, lanes_s, params_range, mle_probs_final_all_veh, max_iter_per_veh)  
...
if __name__ == "__main__":
    ...
    z_min, z_max = 1.2, 1.4
    eps_min, eps_max = 0.2, 0.3
    pol_min, pol_max = 0.5, 0.6
    lam_min, lam_max = 0.5, 1.6

    params_range = [np.arange(z_min, z_max, 0.2), #0.15),
                    np.arange(eps_min, eps_max, 0.1), #0.2), # 0.03),
                    np.arange(pol_min, pol_max, 0.1),
                    np.arange(lam_min, lam_max, 0.5)]  #0.1)]
    print('\params_range\n', np.array(params_range))

    veh_id_minmax = [204, 205] #[210, 220]
    max_iter_per_veh = 2
    ...
    mle_probs_final_all_veh, mle_probs_history_all_veh, marginal_ll_final_all_veh, marginal_ll_history_all_veh, converged_iter_all_veh = \
        run_EM_plot(actual_tracks_dict_all_with_headway, lanes_rxy, lanes_s, max_iter_per_veh, params_range, veh_id_minmax)